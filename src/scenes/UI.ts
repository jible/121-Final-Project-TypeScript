...
export class UI extends Phaser.Scene {
    ...
    displayPlayUI(): void {
        ...
        const interactionButtonConfigs: {
            ...
            },
            {
                ...
                yOffset: 0,
                index: 11,
                onPress: () => (InputHandler.dance = true),
                onRelease: () => (InputHandler.dance = false),
            },
        ]
    ...
    undo() {
        const playScene = this.scene.get('playScene')
        if (playScene instanceof Play) {
            playScene.gameManager.timeLine.undo()
        }
    }
    ...
    redo() {
        const playScene = this.scene.get('playScene')
        if (playScene instanceof Play) {
            playScene.gameManager.timeLine.redo()
        }
    }
